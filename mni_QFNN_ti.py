...
class Qfnn(nn.Module):
    def __init__(self,device) -> None:
        ...
        self.r=nn.Parameter(torch.ones(n_qubits,n_fuzzy_mem)*3)
        self.s=nn.Parameter(torch.ones(n_qubits,n_fuzzy_mem)*4.2)
    ...
    def forward(self,x):
        ...
        for i in range(x.shape[1]):
            a=torch.clamp(torch.min((x[:,i] - self.p[i,0]) / (self.q[i,0] - self.p[i,0]), 
                                    (self.s[i,0] - x[:,i]) / (self.s[i,0] - self.r[i,0])), min=0,max=1)
            b=torch.clamp(torch.min((x[:,i] - self.p[i,1]) / (self.q[i,1] - self.p[i,1]), 
                                    (self.s[i,1] - x[:,i]) / (self.s[i,1] - self.r[i,1])), min=0,max=1)
            fuzzy_list0[:,i]=a
            fuzzy_list1[:,i]=b
